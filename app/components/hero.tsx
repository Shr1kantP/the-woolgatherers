...
export default function Hero() {
  return (
    <section className="relative h-screen w-full bg-[#1D0121]">
      {/* Video frame container — inset from edges to create the framed look */}
      <div className="absolute inset-[40px] sm:inset-[80px] rounded-[8px] overflow-hidden z-0">
        {/* Desktop Video */}
        <video
          autoPlay
          muted
        ...
        {/* Hardware-accelerated blur overlay for performance */}
        <div className="absolute inset-0 z-[1] backdrop-blur-sm bg-black/5 pointer-events-none" />

        {/* Inner edge blur — feathers the video into the frame */}
        <div
          className="absolute inset-0 z-[2] pointer-events-none"
          style={{
            boxShadow: "inset 0 0 40px 20px #1D0121",
          }}
        />
      </div>

      {/* Stacked frame cards */}
      <div className="absolute z-[25] left-1/2 -translate-x-1/2 top-[10%] w-[min(90vw,450px)] sm:left-[4vw] sm:translate-x-0 sm:top-[20%] sm:w-[clamp(240px,22vw,280px)]">
        <Image
      ...
      <button
        ...
        className="absolute z-[26] left-1/2 -translate-x-1/2 top-[40%] w-[min(90vw,450px)] cursor-pointer transition-transform duration-300 hover:scale-[1.02] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F5E9D0] focus-visible:ring-offset-4 focus-visible:ring-offset-black sm:left-[4vw] sm:translate-x-0 sm:top-[50%] sm:w-[clamp(240px,22vw,280px)]"
      >
        <Image
      ...
      {/* <div className="absolute bottom-0 left-0 right-0 z-10 flex justify-center px-2 sm:px-6 pb-[3vh] sm:pb-[1.5vh]">
        <h1
          className="font-heading text-[#F5E9D0] uppercase leading-[0.85] tracking-tight text-center w-full text-[clamp(64px,20vw,64px)] sm:text-[clamp(94px,48vw,132px)]"
        >
          THE WOOLGATHERERS
        </h1>
      </div> */}
