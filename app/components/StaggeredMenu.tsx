...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const numberEls = Array.from(
      panel.querySelectorAll('.sm-panel-item-num')
    ) as HTMLElement[];
    const arrowEls = Array.from(
      panel.querySelectorAll('.sm-panel-item-arrow')
    ) as HTMLElement[];
    ...
    if (arrowEls.length) gsap.set(arrowEls, { opacity: 0, scale: 0.5, rotate: -45 });
    ...
    if (itemEls.length) {
      ...
      if (numberEls.length) {
        tl.to(
          numberEls,
          { opacity: 1, scale: 1, duration: 0.6, ease: 'power2.out', stagger: { each: 0.08, from: 'start' } },
          itemsStart + 0.1
        );
      }
      ...
      if (arrowEls.length) {
        tl.to(
          arrowEls,
          { opacity: 1, scale: 1, rotate: 0, duration: 0.6, ease: 'back.out(1.7)', stagger: { each: 0.08, from: 'start' } },
          itemsStart + 0.15
        );
      }
    }
  ...
  return (
    <div
      className={`sm-scope z-40 ${isFixed ? 'fixed top-0 left-0 w-screen h-[100dvh] pointer-events-none' : 'w-full h-full'}`}
    >
      <div
        className={
      ...
      <style>{`
        ...
        .sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; height: 100%; display: flex; flex-direction: column; overflow-y: auto; z-index: 10; }
        ...
        .sm-scope .sm-panel-inner { display: flex; flex-direction: column; }
        ...
        .sm-scope .sm-panel-item { position: relative; display: inline-flex; text-decoration: none; }
        .sm-scope .sm-panel-itemLabel { display: inline-block; will-change: transform; transform-origin: 50% 100%; }
        
        @media (min-width: 768px) {
          .sm-scope .sm-prelayers { width: 650px; }
        }
