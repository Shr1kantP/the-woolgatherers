...
export default function ImageTrail({ items = [], variant = 1 }: ImageTrailProps): JSX.Element {
  ...
  return (
    <div className="w-full h-full relative z-[100] rounded-lg bg-transparent overflow-visible" ref={containerRef}>
      {items.map((url, i) => (
        <div
          className="content__img absolute top-0 left-0 opacity-0 [will-change:transform,filter]"
          key={i}
        >
          <img
            src={url}
            alt=""
            className="content__img-inner w-auto max-w-[200px] max-h-[200px] object-contain"
          />
        </div>
      ))}
    </div>
